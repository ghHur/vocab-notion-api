...
// ─── 전체 단어 조회 (페이지네이션 포함) ───
export async function getAllWords(apiKey, dbId) {
...
// ─── 단어 추가 ───
export async function createWord(apiKey, dbId, word) {
  const notion = getClient(apiKey);
...
// ─── 단어 수정 ───
export async function updateWord(apiKey, pageId, word) {
  const notion = getClient(apiKey);
...
// ─── 단어 삭제 (아카이브) ───
export async function deleteWord(apiKey, pageId) {
  const notion = getClient(apiKey);
  await notion.pages.update({
    page_id: pageId,
    archived: true,
